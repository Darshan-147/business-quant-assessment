jQuery(document).ready(function ($) {
  if (typeof screenerData === "undefined" || screenerData.error) {
    console.error(
      "Screener data not found or an error occurred:",
      screenerData.error
    );
    return;
  }
  ...
  function addFilterRow() {
    ...
    // Reinitialize Select2 for newly added filters
    $(".metric-select, .unit-select").select2({
  ...
  $(document).on("click", ".delete-filter", function () {
    ...
    // If no filters remain, ensure an empty state is allowed
    if ($(".filter-row").length === 0) {
      table.clear().draw(); // Clear table when all filters are removed
    } else {
  ...
  function applyFilters() {
    ...
    let filteredData = screenerData.data.filter((row) => {
      return filters.every((filter) => {
        let metricIndex = screenerData.list.findIndex(
          (item) => item[0] === filter.metric
        );
